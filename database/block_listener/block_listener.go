...
type BlockListenerDBView interface {
	GetLastBlockNumber(chainId string) (lastBlock *BlockListener, err error)
}
...
func (db blockListenerDB) SaveOrUpdateLastBlockNumber(lastBlock BlockListener) error {
	...
	err := db.gorm.Table(bl.TableName()).Where("chain_id = ?", lastBlock.ChainId).Take(&exitsLastBlock).Error
	if err != nil {
		...
		lastBlock.Updated = uint64(time.Now().Unix())
		updateResult := db.gorm.Table(bl.TableName()).Omit("guid", "created").Where("chain_id = ?", lastBlock.ChainId).Updates(&lastBlock)
...
func (db blockListenerDB) GetLastBlockNumber(chainId string) (lastBlock *BlockListener, err error) {
	bl := new(BlockListener)
	err = db.gorm.Table(bl.TableName()).Where("chain_id = ?", chainId).Take(&lastBlock).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
