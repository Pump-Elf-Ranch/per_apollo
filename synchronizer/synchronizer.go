...
func NewSynchronizer(cfg *Config, db *database.DB, contracts []string, client node.EthClient, shutdown context.CancelCauseFunc) (*Synchronizer, error) {
...
func (syncer *Synchronizer) processBatch(headers []types.Header) error {
	...
	addresses := make([]common.Address, len(syncer.contracts))
	for i := range syncer.contracts {
		addresses[i] = common.HexToAddress(syncer.contracts[i])
	}
	...
	chainContractEvents := make([]common2.ChainContractEvent, 0)
	for i := range logs.Logs {
		...
		if blockNumber != nil {
			chainContractEvent := common2.ChainContractEvent{ContractEvent: common2.ContractEventFromLog(&logs.Logs[i], timestamp, blockNumber)}
